...
void InventoryGridScene::placeTools(int highlightIndex)
{
    ...
    // 2. 遍历显示
    // 注意：只遍历背包容量 (INVENTORY_SIZE) 和 实际物品数量 的较小值
    int count = std::min((int)items.size(), INVENTORY_SIZE);

    for (int i = 0; i < count; i++) {
        Item* item = items[i]; // 获取指针
        if (!item) continue;
        ...
        // 修正：如果 getPrintPos 没数据，用 i 计算默认位置
        // float col = i % 12;
    ...
    float size, float x, float y)
{
    ...
    if (sprite) {
        // ✨ 修改数据获取方式
        auto& items = PlayerState::getInstance()->getInventoryItems();
        if (toolIndex >= items.size()) return; // 安全检查
