...
{
    if (_showToolUseEffect &&_selectedGrid >= 0 && _selectedGrid < _inventoryLayer->INTEM_COUNT)
...
// 显示工具特效 
void InventoryScene::showToolUseEffect()
{
    // 如果没有选择工具，直接返回
    if (_selectedGrid < 0) {
        return;
    ...
    if (_inventoryLayer) {
        ...
        if (!toolImagePath.empty()) {
            ...
            if (_toolUseEffect) {
                ...
                //逆时针旋转30度
                auto rotate = RotateBy::create(0.5f, -30.0f);
                ...
                if (_player->getPlayerDirection() == Direction::RIGHT ||
                    ...
                    rotate = RotateBy::create(0.5f, 30.0f);
                }
                ...
                // 组合动画：旋转 -> 停留 -> 移除
                auto sequence = Sequence::create(rotate, remove, nullptr);
