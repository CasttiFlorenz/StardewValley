...
// 初始化静态成员
SkillLevel::SkillData SkillLevel::s_skillData[SKILL_COUNT];
const char* SkillLevel::LEVEL_STAR_FILLED1 = "/Player/red block.png";
const char* SkillLevel::LEVEL_STAR_EMPTY1 = "/Player/empty block.png";
const char* SkillLevel::LEVEL_STAR_FILLED2 = "/Player/orange block.png";
const char* SkillLevel::LEVEL_STAR_EMPTY2 = "/Player/long block.png";
...
{
    // 初始化技能数据
    s_skillData[(int)SkillType::FARMING] = { "Farming", 0, 4.0f,"/Items/corn.png" };
    s_skillData[(int)SkillType::MINING] = { "Mining", 0,1.2f, "/Items/pickaxe.png" };
    s_skillData[(int)SkillType::FORAGING] = { "Foraging", 0,4.0f, "/Items/cabbage.png" };
    s_skillData[(int)SkillType::FISHING] = { "Fishing", 0, 1.2f,"/Items/anchovy.png" };
...
{
    if ((int)skillType >= 0 && (int)skillType < SKILL_COUNT) {
        s_skillData[(int)skillType].level = level;
    }
}
...
Node* SkillLevel::createLevelStars(int level)
{
    ...
    for (int i = 0; i < MAX_LEVEL; i++) {
        ...
        // 选择星星类型
        const char* filledPath = ((i + 1) % 5 == 0) ? LEVEL_STAR_FILLED2 : LEVEL_STAR_FILLED1;
        const char* emptyPath = ((i + 1) % 5 == 0) ? LEVEL_STAR_EMPTY2 : LEVEL_STAR_EMPTY1;
...
Node* SkillLevel::createSkillItem(SkillType skillType, const Vec2& position)
{
    ...
    SkillData& skill = s_skillData[(int)skillType];
...
{
    if ((int)skillType < 0 || (int)skillType >= SKILL_COUNT || amount <= 0) {
        return false;
    }
...
{
    if ((int)skillType >= 0 && (int)skillType < SKILL_COUNT) {
        return s_skillData[(int)skillType].level;
    }
    return 0;  // 无效类型返回0
}