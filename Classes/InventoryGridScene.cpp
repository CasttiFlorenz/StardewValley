...
void InventoryGridScene::createBackpacks() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // 创建背包精灵
        auto backpack = Sprite::create("/tools/inventory.jpg");
...
void InventoryGridScene::createInvertory()
{
    ...
    // 使用图片背景
    auto background = Sprite::create("/tools/background.png");
...
void InventoryGridScene::placeTools(int highlightIndex)
{
    ...
    // 设置工具缩放参数
    const float HIGHLIGHT_SCALE = 1.7f;  // 高亮时放大
    const float BASE_SCALE = 1.5f;       // 基础大小


    // 遍历工具格
    for (int i = 0; i < INTEM_COUNT; i++) {
        Item& item = _inventory[i];
        float x = backpackLeft0 + _cellWidth * (0.6f + i * 1.0f); // 简单位置计算

        float scale = (i == highlightIndex) ? HIGHLIGHT_SCALE : BASE_SCALE;

        updateToolSprite(i, item.getPos(), scale, x, _backpacks[0].y);
    }
}
    ...
    float size, float x, float y)
{
    ...
    if (sprite) {
        ...
        // 添加到场景中
        this->addChild(sprite, 1);  // Z-order为1，在背景之上
...
void InventoryGridScene::initItem()
{
    ...
    // 放置初始工具
    _inventory[0] = Item("HOE", 1, "/tools/hoe.png");
    _inventory[1] = Item("AXE",1,"/tools/axe.png");
    _inventory[2] = Item("WATERING_CAN", 1,"/tools/wateringCan.png");
    _inventory[3] = Item("PICKAXE", 1,"/tools/pickaxe.png");
    _inventory[4] = Item("SCYTHE", 1, "/tools/scythe.png");
}
